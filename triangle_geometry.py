...
def Kabsch(P, Q, err_tol=10**-12):
    """
    Given two sets of N 3D points with centroid zero, find the rotation matrix 
    ...
    #V, s, WT = scipy.linalg.svd(A)
    #d = np.sign(numpy.linalg.det(np.dot(WT, V)))
    #d = np.sign(numpy.linalg.det(np.dot(WT.T, V.T)))
    I = np.eye(3)
    U = numpy.dot(numpy.dot(V,I),WT)
...
def sample_so3():
    """
    Return a uniformly sampled matrix.
    """
    z = sp.randn(3,3)
    q,r = numpy.linalg.qr(z)
    d = sp.diagonal(r)
    ph = d/sp.absolute(d)
    return sp.multiply(q,ph,q)
